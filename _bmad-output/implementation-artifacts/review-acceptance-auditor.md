...
You are an **Acceptance Auditor** â€” review this diff against the spec and context docs.
...
## Spec: Story 8.1 â€” LinkedIn-style Experience timeline page
...
I want a dedicated Experience page that presents work history in the familiar LinkedIn layout,
so that I can quickly scan career progression and jump straight to case studies.

## Acceptance Criteria

1. Route exists at /experience inside (chrome) route group. Uses metadata API.
2. Sixth DevTools tab: "Experience" pointing at /experience. Uses Briefcase icon. Works in desktop and mobile nav.
3. Schema redesign: company with nested roles, validated with Zod. Fields: company, companyLogo, type, category, location, locationType, org, roles[]. Role fields: name, description, startDate (YYYY-MM), endDate (YYYY-MM or "present").
4. Data re-authored: 4 full-time companies + freelance entries. Buguard has 2 roles. Matches screenshot dates/locations.
5. Durations computed, not stored. LinkedIn-style format ("3 yrs 1 mo", "10 mos"). Uses date-fns. Covers role duration and company total duration.
6. LinkedIn layout with site design tokens. No hardcoded colors. Full-time and Freelance as two labeled sections.
7. Company â†’ filtered Network link via org field. Extends Network filter system with "org" category. URL: /work?org=<project.org>.
8. Robustness: missing logos degrade gracefully. next/image with explicit dims. Keyboard navigable. One h1. Respects prefers-reduced-motion. Not broken at <640px.
9. SEO + tests: /experience in sitemap. Per-route metadata. Duration utility has unit tests. Schema rejects zero roles and invalid endDate.
10. Quality gates pass: typecheck, lint, test:run.
```

## Context Docs
- None referenced in spec frontmatter
...
+  - [x] Define `RoleSchema`: `name` (min 1), `description` (optional), `startDate` (regex `^\d{4}-\d{2}# Code Review Prompt: Acceptance Auditor
